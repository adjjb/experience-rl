...
class OTExperienceBank:
    ...
    def _compute_ot_mapping_gaussian(self, source_dist, target_dist, n_samples=100):
        """计算高斯分布的最优传输映射"""
        # 从两个分布采样
        source_samples = source_dist.rsample((n_samples,)).to(torch.float32)
        target_samples = target_dist.rsample((n_samples,)).to(torch.float32)
    ...
    def _compute_ot_mapping_categorical(self, source_probs, target_probs):
        ...
        for i in range(self.action_dim):
            for j in range(self.action_dim):
                ...
                # 计算JS散度作为成本
                m = 0.5 * (p_i + p_j)
                kl_pi_m = F.kl_div(m.log(), p_i, reduction='sum')
                kl_pj_m = F.kl_div(m.log(), p_j, reduction='sum')
                js_div = 0.5 * (kl_pi_m + kl_pj_m)
                C[i, j] = js_div
        ...
        for _ in range(50):
            u = source_probs.view(-1, 1) / (K @ v + 1e-8)
            v = target_probs.view(-1, 1) / (K.t() @ u + 1e-8)
